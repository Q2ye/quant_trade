...
async def scheduled_alert_retry(
    ...
    try:
        ...
        failure_analysis = await repo.get_failure_analysis(hours=max_retry_hours)

        retried = 0
        for channel, count in failure_analysis.items():
...
async def scheduled_alert_summary(session=None) -> Dict[str, Any]:
    ...
    try:
        ...
        critical_count = await repo.get_critical_alerts_count()
