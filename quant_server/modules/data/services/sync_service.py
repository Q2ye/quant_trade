...
class DataSyncService:
    ...
    def __init__(self, session: AsyncSession, event_engine: Optional[EventEngine] = None):
        ...
        # ETF相关Repository
        self.etf_basic_repo = ETFRepository(session)          # ETF基础信息
        ...
        **kwargs
        ...
        for idx, ts_code in enumerate(ts_codes):
            ...
            await self._update_progress(
                task_id,
                progress=min(100, (idx + 1) / len(ts_codes) * 100),
                ...
                if not adj_df.empty:
                    ...
                    for item in adj_data:
                        ...
                        if existing:
                            ...
                            await self.stock_adj_factor_repo.create(item)
                            records_added += 1
            except Exception as e:
                ...
                if not daily_basic_df.empty:
                    ...
                    for item in daily_basic_data:
                        ...
                        if existing:
                            ...
                            await self.stock_daily_basic_repo.create(item)
                            records_added += 1
            except Exception as e:
    ...
    async def _sync_etf_daily(
        ...
        **kwargs
        ...
        if not start_date:
            start_date = end_date - timedelta(days=30)
        if not ts_codes:
            etfs = await self.etf_basic_repo.get_all_etfs()
    ...
    async def _sync_etf_minute(
        ...
        **kwargs
        ...
        if not start_date:
            start_date = end_date - timedelta(days=7)
        if not ts_codes:
            etfs = await self.etf_basic_repo.get_all_etfs(limit=50)
            ts_codes = [etf.ts_code for etf in etfs]
    ...
    async def _sync_financial_indicator(
        ...
        **kwargs
        ...
        if not end_date:
            end_date = datetime.now().date()
        if not start_date:
            start_date = end_date - timedelta(days=365)  # 财务指标通常按年更新
        if not ts_codes:
            stocks = await self.stock_basic_repo.get_active_stocks()
            ts_codes = [stock.ts_code for stock in stocks]
    ...
    def _estimate_total_items(self, data_type: str, ts_codes: Optional[List[str]] = None) -> int:
        """估算同步项目总数"""
        estimates = {
