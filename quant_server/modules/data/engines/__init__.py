...
async def register_all_data_engines(factory: EngineFactory = None):
    """
    注册所有数据模块引擎到引擎工厂
...
async def create_all_data_engines(
    ...
    try:
        ...
        engines["DATA_CLEAN"] = clean_engine

        # 创建数据质量引擎
        quality_config = configs.get("DATA_QUALITY", {})
        quality_engine = await create_engine_by_type(
            EngineType.DATA_QUALITY,
            config=quality_config,
            event_engine=event_engine
        )
        engines["DATA_QUALITY"] = quality_engine

        # 创建研究引擎
        research_config = configs.get("RESEARCH", {})
        research_engine = await create_research_engine(
            config=research_config,
            event_engine=event_engine
        )
        engines["RESEARCH"] = research_engine

        # 创建数据同步引擎
        sync_config = configs.get("DATA_SYNC", {})
        if create_data_sync_engine:
            sync_engine = await create_data_sync_engine(
                config=sync_config,
                event_engine=event_engine
            )
            engines["DATA_SYNC"] = sync_engine
        else:
            print("警告: 数据同步引擎创建函数不可用，跳过创建")
...
async def create_engine_by_type(
    ...
    **kwargs
    ...
    try:
        ...
        elif engine_type == EngineType.RESEARCH:
...
def get_all_engine_types() -> dict:
    ...
    return {
        "DATA_CLEAN": {
            ...
            "dependencies": [EngineType.EVENT, EngineType.DATA]
        },
        "DATA_QUALITY": {
