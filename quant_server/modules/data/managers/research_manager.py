...
class ResearchManager:
	...
	def register_event_handlers (self) -> None:
		...
		if self.event_engine:
			self.event_engine.register(
				...
				self.handle_research_task_request
			)
			self.event_engine.register(
				"FACTOR_CALCULATION_COMPLETED",
				self.handle_factor_calculation_completed
			)
			self.event_engine.register(
				"FACTOR_ANALYSIS_COMPLETED",
				self.handle_factor_analysis_completed
			)
			self.event_engine.register(
				"RESEARCH_SESSION_STARTED",
				self.handle_research_session_started
			)
			self.event_engine.register(
				"RESEARCH_SESSION_COMPLETED",
				self.handle_research_session_completed
			)
	...
	async def load_factor_library (self) -> None:
		...
		try:
			...
			# 从配置加载因子定义
			factor_definitions = await self.config_manager.get("factor_definitions", {})
	...
	async def execute_factor_calculation (self, task: ResearchTask) -> None:
		...
		try:
			...
			# 通过引擎执行因子计算
			result = await self.research_engine.handle_factor_calculation({
	...
	async def execute_factor_analysis (self, task: ResearchTask) -> None:
		...
		try:
			...
			if missing_factors:
				...
				await self.update_task_progress(task.task_id, 40, f"等待{factor_names}个因子计算完成")
				return
			...
			# 执行因子分析
			result = await self.research_engine.handle_factor_analysis({
	...
	async def execute_factor_optimization (self, task: ResearchTask) -> None:
		...
		try:
			...
			# 执行因子优化
			optimization_result = await self.research_engine.execute_factor_optimization(
	...
	async def execute_factor_backtest (self, task: ResearchTask) -> None:
		...
		try:
			...
			# 执行因子回测
			backtest_result = await self.research_engine.execute_factor_backtest(
				task.task_id, parameters
			)
	...
	async def execute_factor_selection (self, task: ResearchTask) -> None:
		...
		try:
			...
			# 执行因子选择
			selection_result = await self.research_engine.execute_factor_selection(
				task.task_id, parameters
			)
	...
	def create_factor_cache_key (self, factor_name: str, parameters: Dict) -> str:
		"""
		创建因子缓存键
	...
	def parameters_match (self, stored_params: Dict, requested_params: Dict) -> bool:
		"""
		检查参数是否匹配
	...
	async def generate_analysis_report (self, analysis_result: Dict, parameters: Dict) -> Dict:
		"""
		生成分析报告
	...
	async def validate_optimization_result (self, optimization_result: Dict, parameters: Dict) -> Dict:
		"""
		验证优化结果
	...
	async def analyze_backtest_result (self, backtest_result: Dict) -> Dict:
		"""
		分析回测结果
	...
	async def validate_selection_result (self, selection_result: Dict, parameters: Dict) -> Dict:
		"""
		验证选择结果
			...
			if task_id in self.active_tasks:
				...
				if task.parent_task_id and task.parent_task_id in self.active_tasks:
					...
					all_subtasks_completed = all(
						subtask_id not in self.active_tasks or
						self.active_tasks[subtask_id].status == "completed"
