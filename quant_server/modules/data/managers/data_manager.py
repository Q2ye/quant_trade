...
class DataManager:
    ...
    async def execute_research_task(self, task: DataTask, params: Dict) -> None:
        ...
        try:
            ...
            result = await self.research_engine.handle_research_request(
                DataResearchStartedEvent(**params)
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sync_engine_status": self.sync_engine.status if self.sync_engine else "not_initialized",
            "clean_engine_status": self.clean_engine.status if self.clean_engine else "not_initialized",
            "quality_engine_status": self.quality_engine.status if self.quality_engine else "not_initialized",
            "research_engine_status": self.research_engine.status if self.research_engine else "not_initialized",
