...
class DependencyManager:
    ...
    def __init__(self):
        ...
        self._dependency_graph: Dict[int, Dict[int, Dependency]] = {}

        # 逆向依赖图 {strategy_id: {dependency_strategy_id}}
        self._reverse_graph: Dict[int, Set[int]] = {}
    ...
    def resolve_load_order(self) -> List[int]:
        """
        解析策略加载顺序（拓扑排序）
    ...
    def _calculate_in_degree(self) -> Dict[int, int]:
        """
        计算每个节点的入度
    ...
    def _has_circular_dependency(self) -> bool:
        ...
        def visit(strategy_id: str) -> bool:
            ...
            # 访问依赖
            for dependent_id in self._reverse_graph.get(strategy_id, set()):
                if dependent_id not in visited:
                    if visit(dependent_id):
                        return True
                elif dependent_id in rec_stack:
                    # 发现循环
                    logger.error(f"发现循环依赖: {strategy_id} -> {dependent_id}")
                    return True

            rec_stack.remove(strategy_id)
            return False
