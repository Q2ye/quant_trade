...
class AIEngine(EngineBase):
    ...
    async def load_strategy(
        ...
        # 加载模型
        if model_path:
            model = await self._load_model(strategy_id, model_path)
    ...
    async def process_bar(
        ...
        try:
            # 更新特征
            await self._update_features(strategy_id, bar_data)

            # 进行预测
            prediction = await self._predict(strategy_id, bar_data)
