...
from quant_server.modules.strategy.models import StrategyInstance, StrategyConfig
...
class StrategyService:
    ...
    async def update_strategy(
        ...
        try:
            ...
            # 检查状态
            if strategy.status in [StrategyStatus.RUNNING]:
                return {
                    "success": False,
                    "error": "策略运行中，无法修改",
    ...
    async def compile_strategy(
        ...
        try:
            ...
            await self.strategy_repo.update(strategy_id, {
                "status": StrategyStatus.COMPILED.value,
                "updated_at": datetime.now()
            })
            ...
            return {
                "success": True,
                "data": {"id": strategy_id, "status": StrategyStatus.COMPILED.value}
            }
        except Exception as e:
    ...
    def _to_dict(self, strategy) -> Dict[str, Any]:
        """转换为字典"""
        return {
            "id": strategy.id,
            "name": strategy.name,
