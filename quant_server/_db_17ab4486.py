...
实盘修复记录（2026-08「实盘运行发现的问题修复」提交，均修实盘 bug、非改回测）：
  1. 防补仓/重复入池：_confirmed_active 集合（选股排除「已确认未卖出」的票）
     + _persist_candidate 落库前校验（已持仓/未了结买入意图 → 拒绝落库）
  2. 满仓候选永不确认（8-25 603565）：_confirm_pending_buys 改两阶段
     （先确认所有候选，再按仓位处理）
  3. 同日确认误拒（8-24 603519/603565）：signal_date T+1 守卫
     （同日候选跳过确认，待次日收盘确认）
  4. 出场 0 股卖不掉：exit quantity = 实际持仓股数（全平）
  5. 候选状态竞态：_rejected_candidate_ids 跳过本批次已拒候选
  6. 信号时间戳 00:00：改用北京时间具体时分秒
  7. 年线门口径不统一：统一到 shared.market_regime.compute_regime
     （annual_gate_band 参数化；band=0.0 恢复旧版二元）
  ※ 其中 1/2/3/7 会连带改变回测行为（选股排除/确认时机/满仓处理/regime），
    是 354%→289% 回测回归的排查对象（见顶部 AB_* A/B 开关）

修复方案评审结论（2026-08-30 评审，标注待修正项）：
  1. 防补仓过度：补仓信号本身合理，_confirmed_active 锁死了合理补仓 → 待放开
  2. 候选入池应判定信号强度，而非仅价格确认 → 待改
  3. 同日信号/次日确认本就是正常流程，T+1 守卫针对的「误拒」非真问题 → 待重新审视
  4. 出场数量用实际持仓数 → 正确，保留
  5. 已拒股票应重新计算再决定是否入池，而非跳过 → 待改
  6. 时间戳用北京时间 → 正确，保留
  7. 年线门统一 band 参数化；band=0.0 恢复二元（详见上文）
...
# =========================================================================
# 临时 A/B 开关 — 排查「7.1 基准 → 现在」回测回归用（定位后删除）
#   True = 当前行为；False = 回退到 7.1 基准行为
# =========================================================================
AB_CONFIRMED_ACTIVE = False   # 改动1：选股排除 _confirmed_active（防补仓）——基准测试：回退
AB_T1_GUARD = False           # 改动2：signal_date T+1 守卫（同日误拒修复）——基准测试：回退
AB_TWO_PHASE = False          # 改动3：两阶段确认（满仓不跳过，保留待空位）——基准测试：回退
...
class HighVolMomentumStrategy(BaseStrategy):
    ...
    DEFAULT_PARAMS: Dict[str, Any] = {
        ...
        "annual_gate_band": 0.0,         # 年线门偏离阈值（0=恢复旧版二元：close<MA250 即熊市降仓，无带滞回）
        ...
        # —— v7.3 震荡分支（折中仓位，介于熊市降仓与牛市满仓之间） ——
        "range_max_positions": 1,          # 震荡最大持仓数（熊市1 → 震荡1 → 牛市2）
        "range_single_weight": 0.5,        # 震荡单票权重（熊市25% → 震荡50% → 牛市50%）
    ...
    def __init__(
        ...
        # v7.3 震荡分支
        self.range_max_positions = int(merged.get("range_max_positions", 1))
        self.range_single_weight = float(merged.get("range_single_weight", 0.5))
        # 运行
    ...
    def _run_rebalance(self) -> List[TradingSignal]:
        ...
        bear = regime == 0
        # 持仓上限与单票权重：熊市降仓（1×25%）/ 震荡折中（1×50%）/ 牛市满仓（2×50%）
    ...
    def _screen_stocks(self) -> List[str]:
        ...
        for code in self._data_cache.keys():
            if code in self._holdings or code in self._exit_pending or (AB_CONFIRMED_ACTIVE and code in self._confirmed_active):
                continue
    ...
    def _confirm_pending_buys(self) -> List[TradingSignal]:
        ...
        self._buy_pending.clear()
        # v7.1: 判断当前 regime（决定持仓上限/权重上限与 is_bear 标志）
        ...
        for code, pinfo in pending.items():
            ...
            if AB_T1_GUARD and _sig_date and _sig_date >= today:
        ...
        for code, pinfo, price in passed:
            if len(self._holdings) >= eff_max_pos:
                ...
                continue
            signal_price = float(pinfo["signal_price"])
            # P1-1 修复：按当前 regime 封顶权重（跨行情候选：入池权重不得超当前 regime 上限）
    ...
    def _position_sizing(self, regime: int) -> Tuple[int, float]:
        """按 regime 返回 (最大持仓数, 单票权重)：熊市降仓 / 震荡折中 / 牛市满仓。"""
        if regime == 0:
            return self.bear_max_positions, self.bear_single_weight
        if regime == 1:
            return self.range_max_positions, self.range_single_weight
        return self.max_positions, self.max_single_weight
