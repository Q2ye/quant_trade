...
class SystemNotification(Base):
    ...
    recipient_id = Column(Integer, ForeignKey('sys_users.id'), comment='接收用户ID')
...
class UserPreference(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True, comment='偏好设置ID')
    user_id = Column(Integer, ForeignKey('sys_users.id'), nullable=False, unique=True, comment='用户ID')
...
class RetentionPolicyLog(Base):
	...
	id = Column(Integer, primary_key=True, autoincrement=True, comment='日志ID')
	policy_id = Column(Integer, ForeignKey('retention_policies.id'), nullable=False, comment='策略ID')
...
class ChunkMetadata(Base):
	...
	id = Column(Integer, primary_key=True, autoincrement=True, comment='分片ID')
	table_name = Column(String(100), ForeignKey('hyper_table_metadata.table_name'), nullable=False, comment='表名')
	...
	merged_into = Column(Integer, comment='合并到哪个分片')
