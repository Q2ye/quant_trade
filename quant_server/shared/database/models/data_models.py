...
class StockDailyBasic(Base):
	...
	turnover_rate = Column(Numeric(18, 6), comment='换手率（%）')
	turnover_rate_f = Column(Numeric(18, 6), comment='流通股换手率（%）')
	volume_ratio = Column(Numeric(18, 6), comment='量比')
	pe = Column(Numeric(18, 6), comment='市盈率（总市值/净利润）')
	pe_ttm = Column(Numeric(18, 6), comment='市盈率（TTM）')
	pb = Column(Numeric(18, 6), comment='市净率（总市值/净资产）')
	ps = Column(Numeric(18, 6), comment='市销率')
	ps_ttm = Column(Numeric(18, 6), comment='市销率（TTM）')
	dv_ratio = Column(Numeric(18, 6), comment='股息率（%）')
	dv_ttm = Column(Numeric(18, 6), comment='股息率（TTM）')
...
class StockMoneyflow(Base):
	...
	buy_sm_amount = Column(Numeric(18, 6), comment='小单买入金额（万元）')
	sell_sm_vol = Column(Integer, comment='小单卖出量（手）')
	sell_sm_amount = Column(Numeric(18, 6), comment='小单卖出金额（万元）')
	buy_md_vol = Column(Integer, comment='中单买入量（手）')
	buy_md_amount = Column(Numeric(18, 6), comment='中单买入金额（万元）')
	sell_md_vol = Column(Integer, comment='中单卖出量（手）')
	sell_md_amount = Column(Numeric(18, 6), comment='中单卖出金额（万元）')
	buy_lg_vol = Column(Integer, comment='大单买入量（手）')
	buy_lg_amount = Column(Numeric(18, 6), comment='大单买入金额（万元）')
	sell_lg_vol = Column(Integer, comment='大单卖出量（手）')
	sell_lg_amount = Column(Numeric(18, 6), comment='大单卖出金额（万元）')
	buy_elg_vol = Column(Integer, comment='特大单买入量（手）')
	buy_elg_amount = Column(Numeric(18, 6), comment='特大单买入金额（万元）')
	sell_elg_vol = Column(Integer, comment='特大单卖出量（手）')
	sell_elg_amount = Column(Numeric(18, 6), comment='特大单卖出金额（万元）')
	net_mf_vol = Column(Integer, comment='净流入量（手）')
	net_mf_amount = Column(Numeric(18, 6), comment='净流入金额（万元）')
	created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), comment='创建时间')
	updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc),
	                    onupdate=lambda: datetime.now(timezone.utc), comment='更新时间')
...
class FinancialIncome(Base):
    ...
    basic_eps = Column(Numeric(18, 6), comment='基本每股收益')
    diluted_eps = Column(Numeric(18, 6), comment='稀释每股收益')
...
class EtfBasic(Base):
	...
	list_status = Column(String(1), comment="上市状态: L=上市 D=退市 P=待上市")
...
class IndexDaily(Base):
	...
	ts_code = Column(String(20), primary_key=True, comment='指数代码')
	trade_date = Column(DateTime, primary_key=True, comment='交易日期')
	close = Column(Numeric(18, 6), nullable=False, comment='收盘价')
...
class FactorData(Base):
	...
	# factor_definition_id — 待 DB 迁移添加列后启用
	...
	# definition relationship — 待 factor_definition_id 列迁移完成后启用
	# 索引
	__table_args__ = (
...
class FactorDefinition(Base):
	...
	# factor_values relationship — 待因子数据表单向 FK 迁移完成后启用
...
class StockForecast(Base):
	...
	p_change_min = Column(Numeric(18, 6), comment='净利润变动下限(%)')
	p_change_max = Column(Numeric(18, 6), comment='净利润变动上限(%)')
...
class StockExpress(Base):
	...
	diluted_eps = Column(Numeric(18, 6), comment='稀释每股收益')
	diluted_roe = Column(Numeric(18, 6), comment='净资产收益率(%)')
	yoy_net_profit = Column(Numeric(16, 4), comment='净利润同比(%)')
	bps = Column(Numeric(18, 6), comment='每股净资产')
	...
	eps_last_year = Column(Numeric(18, 6), comment='去年同期每股收益')
	open_net_assets = Column(Numeric(18, 4), comment='期初净资产')
	open_bps = Column(Numeric(18, 6), comment='期初每股净资产')
...
class StockDividend(Base):
	...
	stk_bo_rate = Column(Numeric(18, 6), comment='每股转增')
	stk_co_rate = Column(Numeric(18, 6), comment='每股送股')
...
class EtfShare(Base):
	...
	fund_size = Column(Numeric(18, 4), comment='总份额（万份）')
	fund_vol = Column(Numeric(18, 4), comment='总规模（万元）')
	created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
	updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
...
class StockShareFloat(Base):
	...
	float_ratio = Column(Numeric(18, 6), comment='流通股份占总股本比率')
...
class StockPledgeStat(Base):
	...
	pledge_ratio = Column(Numeric(18, 6), comment='质押比例(%)')
	created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
	updated_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc),
	                    onupdate=lambda: datetime.now(timezone.utc))
...
class StockStkHoldertrade(Base):
	...
	change_ratio = Column(Numeric(18, 6), comment='变动比例(%)')
	after_share = Column(Numeric(18, 2), comment='变动后持股')
	after_ratio = Column(Numeric(18, 6), comment='变动后持股比例(%)')
	avg_price = Column(Numeric(18, 6), comment='增/减持均价')
...
class IndexSwDaily(Base):
	...
	open = Column(Numeric(18, 6), comment='开盘点位')
	low = Column(Numeric(18, 6), comment='最低点位')
	high = Column(Numeric(18, 6), comment='最高点位')
	close = Column(Numeric(18, 6), comment='收盘点位')
	change = Column(Numeric(18, 6), comment='涨跌点位')
	pct_change = Column(Numeric(18, 6), comment='涨跌幅')
...
class IndexDailyBasic(Base):
	...
	turnover_rate = Column(Numeric(18, 6), comment='换手率(%)')
	turnover_rate_f = Column(Numeric(18, 6), comment='自由流通换手率(%)')
	pe = Column(Numeric(18, 6), comment='市盈率')
	pe_ttm = Column(Numeric(18, 6), comment='市盈率(TTM)')
	pb = Column(Numeric(18, 6), comment='市净率')
	created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
...
class StockForecastPro(Base):
	...
	eps = Column(Numeric(18, 6), comment='预测每股收益')
	pe = Column(Numeric(18, 6), comment='预测市盈率')
	rd = Column(Numeric(18, 6), comment='预测研发费用')
	roe = Column(Numeric(18, 6), comment='预测净资产收益率')
	ev_ebitda = Column(Numeric(18, 6), comment='预测EV/EBITDA')
	rating = Column(String(50), comment='评级')
	max_price = Column(Numeric(18, 6), comment='目标最高价')
	min_price = Column(Numeric(18, 6), comment='目标最低价')
...
class IndexWeekly(Base):
	...
	high = Column(Numeric(18, 6), comment='最高点位')
	low = Column(Numeric(18, 6), comment='最低点位')
	pre_close = Column(Numeric(18, 6), comment='昨日收盘点')
	change = Column(Numeric(18, 6), comment='涨跌点位')
