...
async def get_risk_alerts_api (
	...
	"""
	try:
		logger.info(f"用户 {current_user.get('username')} 请求风险警报列表")
...
async def monitor_module_health_check (
	...
	"""
	try:
		logger.info(f"用户 {current_user.get('username')} 请求监控模块健康检查")

		health_status = await check_monitor_module_health(
			session=db_session,
		)
