...
class QueuedEvent:
	...
	event: Any = field(compare=False)  # 事件对象（不参与排序）
...
class EventHandler:
	...
	async def call (self, event: Any) -> Any:
		"""调用处理器
...
class EventEngine(EngineBase):
	...
	def __init__ (self, config: EngineConfigEntity, event_engine=None):
		...
		# 事件历史
		self._event_history: Deque[Any] = deque(maxlen=10000)  # 最多保存10000个事件
	...
	async def _get_event (self, timeout: float = 1.0) -> Optional[Any]:
		"""获取事件
	...
	def register (self,
		...
		Raises:
			ValueError: 参数无效
		"""
		# 运行时类型检查，确保handler是可调用对象
		# 虽然有类型注解，但运行时仍需验证
		assert callable(handler), "处理器必须是可调用对象"
