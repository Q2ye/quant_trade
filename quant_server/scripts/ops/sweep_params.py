# -*- coding: utf-8 -*-
"""
⚠️ 已废弃（2026-09-12）—— 请勿再使用本脚本产出的参数结论。

废弃原因（2026-09 代码审计确认）：
  1) 在**含测试期的全区间**上扫描参数并选优，无训练/测试切分、无 walk-forward
     → 「最优参数」为过拟合结果。`bottom_strategy.py` 中若干 regime 参数的
       注释自称「2026-08 实测最优」，即出自此类全区间扫描。
  2) 本脚本只 glob `etf_bottom_v1_*.joblib`（实际取到 v1_backup，42 特征 / 阈值 0.60），
