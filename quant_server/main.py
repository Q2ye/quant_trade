...
from quant_server.utils.core_utils.logging_utils import (
    ...
    # 上下文管理
    get_context_manager, with_context, LogContextManager,

    # 格式化器
    FormatterFactory, LogFormatter,
    ...
    get_global_log_manager, get_global_context_manager,
...
from quant_server.shared.config.settings import (
    ...
    reload_settings, validate_config
)
...
class ModuleConfig:
    ...
    def validate(self) -> bool:
        ...
        if not isinstance(self.dependencies, list):
            logger.error(f"模块 {self.name} 依赖项必须是列表类型")
            return False
...
class StartupConfig:
    ...
    @with_context(operation="load_startup_config", source="config_loader")
    ...
    @with_context(operation="apply_config_data", source="config_loader")
    def _apply_config_data(self, config_data: Dict[str, Any]):
        """应用配置数据"""
        # 系统配置
        system_config = config_data.get('system', {})
        server_config = config_data.get('server', {})
        engine_config = config_data.get('engines', {})
        ...
        # 基础配置
        self.system_name = system_config.get('name', self.settings.APP_NAME)
        self.version = system_config.get('version', self.settings.APP_VERSION)
        self.mode = system_config.get('mode', self.settings.ENVIRONMENT.value)

        # 服务器配置
        self.host = server_config.get('host', self.settings.API.HOST)
        self.port = server_config.get('port', self.settings.API.PORT)
        self.workers = server_config.get('workers', 1)

        # 引擎配置
        self.auto_start_main_engine = engine_config.get('auto_start_main_engine', True)
        self.auto_start_event_engine = engine_config.get('auto_start_event_engine', True)
        self.max_workers = engine_config.get('max_workers', 10)
        self.queue_size = engine_config.get('queue_size', 10000)

        # 功能配置
        self.enable_web_socket = system_config.get('enable_web_socket', True)
        self.enable_monitoring = system_config.get('enable_monitoring', True)
        self.enable_health_check = system_config.get('enable_health_check', True)
        self.shutdown_timeout = system_config.get('shutdown_timeout', 30.0)
        self.log_level = system_config.get('log_level', self.settings.LOG.LEVEL)
...
class QuantServer:
    ...
    def _setup_logging(self):
        ...
        try:
            ...
            logger.warning(f"无效的日志级别: {log_level_str}, 使用默认级别: INFO")
    ...
    def _log_configuration(self):
        """记录配置信息"""
        config_info = self.config.get_system_config()
    ...
    async def _load_module(self, module_name: str) -> Optional[Any]:
        """动态加载模块"""
        try:
    ...
    async def _sort_modules_by_dependency(self) -> List[str]:
        ...
        # 对每个未访问的节点执行DFS
        for node in self.enabled_modules:
