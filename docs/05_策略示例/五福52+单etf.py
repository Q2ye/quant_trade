...
# 克隆自聚宽文章：https://www.joinquant.com/post/79178
# 标题：【研究分享】多只 ETF 归一净值对比工具
# 作者：张工学量化
...
def initialize(context):
	...
	g.normal_holdings_num = 1
	g.weak_holdings_num = 1
	...
	run_daily(early_sell_check_1020, time='10:20')
...
def execute_buy_trades(context):
	...
	log.info(f"计划买入ETF: {g.pending_buy_etfs}，先进行13:10趋势判断；若不满足，将在13:40/14:10/14:40复检，14:55强制买入")
...
def early_sell_check_1020(context):
	"""10:20早盘动量检查：如果当前持仓在10:20排名中将被卖出，
	且盘中斜率向上+价格高于开盘价，则提前卖出。
	如果价格低于开盘价则跳过，等13:10正常处理。"""
	positions = [s for s, p in context.portfolio.positions.items() if p.total_amount > 0]
	if not positions:
		return

	# 运行10:20动量排名
	calculate_and_log_ranked_etfs(context)
	ranked = getattr(g, 'ranked_etfs_result', [])
	target_holdings = get_target_holdings_num()
	target_codes = {m['etf'] for m in ranked[:target_holdings]} if ranked else set()

	# 找出"今天会被卖出"的持仓（不在top-N目标中）
	to_sell = [s for s in positions if s not in target_codes]
	if not to_sell:
		return

	log.info("★" * 80)
	log.info("▶️ 【10:20早盘卖出检查】检查是否有可提前卖出的ETF...")
	current_data = get_current_data()
	for etf in to_sell:
		etf_name = get_security_name(etf)
		price = current_data[etf].last_price
		open_price = current_data[etf].day_open

		# 条件A: 价格必须高于开盘价
		if price <= open_price:
			log.info(f"[10:20早盘] {etf} {etf_name} 价格{price:.4f} <= 开盘{open_price:.4f}，跳过")
			continue

		# 条件B: 盘中斜率必须为正（上涨趋势）
		if not check_intraday_trend(etf, context):
			log.info(f"[10:20早盘] {etf} {etf_name} 盘中趋势未确认上涨，跳过")
			continue

		# 两个条件都满足 → 提前卖出
		log.info(f"[10:20早盘] ✅ {etf} {etf_name} 斜率向上+价格>开盘，提前卖出！")
		smart_order_target_value(etf, 0, context)

	log.info("⏸️ 【10:20早盘卖出检查】执行完毕！")
