...
| ExecutionEngine | `execution_engine.py` | `execute_signal()` 组装订单 → `execute_order()` 下单（`simulated_trading` 开关，见 :326 订正）→ 成交后更新持仓、触发结算、订单 DB 持久化 | BrokerAdapter（**两分支均回退 SimBrokerAdapter，无真实券商实现**）、PositionEngine、RiskEngine、trade_manager、session_factory |
...
- 半自动交易模式（人工确认）与 AGENTS.md「市场与交易模式」一致。`config.yaml` 的 `simulated_trading` 开关在 ExecutionEngine 生效（`execution_engine.py:166-203`）：模拟路径内存直接成交，「真实」路径调 `broker_adapter.send_order`——但**两条路径的 broker 都是 `SimBrokerAdapter`，系统无真实下单能力**。⚠️ **2026-09-12 订正**：`.env` 的 `SIMULATED_TRADING` 仅影响日志/API 展示，不是执行开关；真实资金风险只在人工环节。
- 事件命名遵循 `{module}.{domain}.{action}.{status}` 规范（如 `risk.check.requested`、`strategy.signal.confirmed`）。
- 数据表（signals/orders/trades/trade_fees/positions）结构见 `docs/sql/create_table.sql`（表结构设计详见 `docs/04-归档/design/量化交易平台数据表设计.md`，已归档）。
