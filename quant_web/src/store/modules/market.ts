...
const mutations = {
  ...
  SET_STOCK_BASIC(state: MarketState, stocks: StockBasic[]) {
    ...
    state.stockBasicMap = new Map(
      stocks.map((stock) => [stock.ts_code, stock]),
    );
  },
  ...
  SET_INDEX_DATA(
    state: MarketState,
    payload: { index: string; data: IndexData },
  ) {
    state.indexData[payload.index as keyof typeof state.indexData] =
      payload.data;
  },
  ...
  UPDATE_MARKET_STATUS(
    state: MarketState,
    status: MarketState["marketStatus"],
  ) {
    state.marketStatus = status;
  },

  // 设置板块表现
  SET_SECTOR_PERFORMANCE(
    state: MarketState,
    sectors: MarketState["sectorPerformance"],
  ) {
...
const actions = {
  ...
  async loadStockBasic({ commit, state }: any) {
    if (
      state.stockBasic.length > 0 &&
      Date.now() - state.lastUpdate < 3600000
    ) {
      return; // 1小时内不重复加载
    }
    ...
    try {
      const response = await fetch("/quantTrade/market/stock-basic");
  ...
  async subscribeRealTime({ commit, rootState }: any, symbols: string[]) {
    ...
    try {
      ...
      if (rootState.websocket.isConnected) {
        rootState.websocket.send("subscribe", {
          channels: symbols.map((s) => `quote_${s}`),
        });
      }
  ...
  async unsubscribeRealTime({ commit, rootState }: any, symbols: string[]) {
    try {
      // 通过WebSocket取消订阅
      if (rootState.websocket.isConnected) {
        rootState.websocket.send("unsubscribe", {
          channels: symbols.map((s) => `quote_${s}`),
        });
      }
  ...
  async searchStocks({ state, dispatch }: any, keyword: string) {
    ...
    return state.stockBasic
      .filter((stock: StockBasic) => {
        return (
