...
export function createPersistedState(
  ...
  return (store) => {
    ...
    if (savedState) {
      // 合并状态
      store.replaceState(
        overwrite ? savedState : merge(store.state, savedState, arrayMerger),
      );
    }
...
export const quantPersistedState = createPersistedState({
  ...
  arrayMerger: (state: any[], saved: any[]) => {
    ...
    if (state && state.length > 0 && saved && saved.length > 0) {
      ...
      const unique = Array.from(
        new Map(merged.map((item) => [item.symbol || item.id, item])).values(),
      );
  ...
  rehydrated: (store) => {
    ...
    if (store.state.user.token) {
      // WebSocket重连由 useWebSocket composable 在组件层处理
      // 此处仅做状态标记，不直接调用不存在的 websocket 模块
      store._vm?.$root?.$emit?.("reconnect-websocket");
    }
  },
});
