...
import { OrderRequest, Trade, Position } from '@/types/trading'
...
export function useTrade() {
  ...
  const quickOrder = async (order: {
    ...
    try {
      ...
      // 添加到待处理订单列表
      pendingOrders.value.push(orderRequest)

      // 发送订单到服务器
      send('place_order', orderRequest)

      // 更新本地状态
      store.commit('trade/ADD_PENDING_ORDER', orderRequest)
  ...
  // 市价单
  const marketOrder = (ts_code: string, direction: 'buy' | 'sell', volume: number) => {
    return quickOrder({
      ts_code,
      direction,
      order_type: 'market',
  ...
  // 限价单
  const limitOrder = (ts_code: string, direction: 'buy' | 'sell', price: number, volume: number) => {
    return quickOrder({
      ts_code,
      direction,
      order_type: 'limit',
  ...
  const cancelAllOrders = async (symbol?: string) => {
    ...
    const results = await Promise.allSettled(
      orders.map(order => cancelOrder(order.order_id))
    )
  ...
  const validateOrder = async (order: any) => {
    ...
    if (order.direction === 'buy') {
      ...
      const estimatedCost = order.volume * (order.price || store.getters['market/getCurrentPrice'](order.ts_code))
  ...
  // 获取交易费用估算
  const estimateFees = (price: number, volume: number, direction: 'buy' | 'sell') => {
    ...
    return {
      ...
      netAmount: direction === 'buy' ? amount + commission + transferFee : amount - commission - tax - transferFee
    }
  }
