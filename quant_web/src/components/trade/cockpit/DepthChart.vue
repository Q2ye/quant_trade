  ...
  const option = {
    ...
    series: [
      // 买盘深度
      {
        name: '买盘',
        type: 'bar',
        data: depthData.value.bids.map(item => [item.price, item.volume, item.total]),
        itemStyle: {
          color: '#00b36a'
        },
        emphasis: {
      ...
      // 卖盘深度
      {
        name: '卖盘',
        type: 'bar',
        data: depthData.value.asks.map(item => [item.price, item.volume, item.total]),
        itemStyle: {
          color: '#ff4d4d'
        },
        emphasis: {
...
const handleDepthUpdate = (data: any) => {
  ...
  const bids = data.bids.map((bid: [number, number], index: number) => {
    const total = data.bids.slice(0, index + 1).reduce((sum: number, b: [number, number]) => sum + b[1], 0)
    return {
      price: bid[0],
      volume: bid[1],
      total,
  ...
  const asks = data.asks.map((ask: [number, number], index: number) => {
    const total = data.asks.slice(0, index + 1).reduce((sum: number, a: [number, number]) => sum + a[1], 0)
    return {
      price: ask[0],
      volume: ask[1],
      total,
...
<template>
  <div class="depth-chart">
    ...
    <div class="depth-info" v-if="depthData.bids.length > 0 && depthData.asks.length > 0">
      <div class="info-row">
        <span>买一: {{ depthData.bids[0]?.price.toFixed(2) }} ({{ depthData.bids[0]?.volume }})</span>
        <span>卖一: {{ depthData.asks[0]?.price.toFixed(2) }} ({{ depthData.asks[0]?.volume }})</span>
      </div>
      <div class="info-row">
        <span>价差: {{ (depthData.asks[0]?.price - depthData.bids[0]?.price).toFixed(2) }}</span>
        <span>深度比例: {{ (depthData.bids[0]?.volume / depthData.asks[0]?.volume).toFixed(2) }}</span>
      </div>
    </div>
  </div>
</template>
