<!-- components/portfolio/PortfolioOptimization.vue -->
<!--组合优化-->
<!--有效前沿图表-->
<!--权重对比表格-->
<template>
    ...
    <el-row :gutter="16">
        ...
        <el-card>
          <template #header>
            <div class="card-header">
              <iconify-icon icon="mdi:chart-bubble" width="20" />
              <span>有效前沿</span>
            </div>
          </template>
      ...
      <el-col :span="12">
        <el-card>
          <template #header>
            <div class="card-header">
              <iconify-icon icon="mdi:weight" width="20" />
              <span>权重优化</span>
            </div>
          </template>
          <div class="optimization-controls">
            <el-form :model="optimizationForm" label-width="120px">
              <el-form-item label="优化目标">
                <el-select v-model="optimizationForm.objective" placeholder="请选择优化目标">
                  <el-option label="最大化夏普比率" value="sharpe" />
                  <el-option label="最小化波动率" value="volatility" />
                  <el-option label="最大化收益" value="return" />
              ...
              <el-form-item label="最大回撤约束">
                <el-input-number
                  v-model="optimizationForm.maxDrawdown"
                  :min="0"
                  :max="30"
                  :step="0.1"
                  controls-position="right"
              ...
              <el-form-item label="单个资产上限">
                <el-input-number
                  v-model="optimizationForm.maxWeight"
                  :min="0"
                  :max="100"
                  :step="1"
                  controls-position="right"
              ...
              <el-form-item>
                <el-button type="primary" @click="runOptimization" :loading="optimizing">
                  <iconify-icon icon="mdi:rocket-launch" width="16" />
          ...
          <div class="weight-comparison">
            <h4>权重对比</h4>
            <el-table :data="weightComparison" stripe style="width: 100%">
              ...
              <el-table-column prop="optimized" label="优化权重" width="100">
                <template #default="scope">
                  {{ scope.row.optimized }}%
                </template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="80">
                <template #default="scope">
                  <el-button
                    v-if="scope.row.optimized !== scope.row.current"
                    type="primary"
                    link
                    @click="applyWeight(scope.row.asset)"
                  >
                    应用
                  </el-button>
                </template>
      ...
      <el-col :span="24">
        <el-card>
          ...
          <div class="optimization-results">
            <el-row :gutter="16">
              <el-col :span="6" v-for="metric in optimizationMetrics" :key="metric.name">
                <div class="metric-card">
                  <div class="metric-name">{{ metric.name }}</div>
                  <div class="metric-values">
                    <div class="current-value">
                      <span class="label">当前:</span>
                      <span :class="metric.currentClass">{{ metric.current }}</span>
                    </div>
                    <div class="optimized-value">
                      <span class="label">优化:</span>
                      <span :class="metric.optimizedClass">{{ metric.optimized }}</span>
                    </div>
                  </div>
                  <div class="improvement" :class="metric.improvement >= 0 ? 'positive' : 'negative'">
...
const runOptimization = () => {
  ...
  setTimeout(() => {
    ...
    // 重新计算总权重为100%
    const total = weightComparison.value.reduce((sum, item) => sum + item.optimized, 0)
...
const applyWeight = (asset: string) => {
  ...
  if (item) {
    ...
    item.change = 0
    ElMessage.success(`已应用 ${asset} 的优化权重`)
  }
}
    ...
    const option = {
      tooltip: {
        ...
        formatter: function (params: any) {
          return `${params.data[4]}<br/>风险: ${params.data[0].toFixed(1)}%<br/>收益: ${params.data[1].toFixed(1)}%`
        }
      },
      grid: {
        left: '3%',
