    ...
    render: (row) => {
      return h("div", { class: "strength-cell" }, [
        ...
        h("div", { class: "strength-info" }, [
          h(
            NText,
            { depth: 1, class: "strength-value" },
            { default: () => row.strength.toFixed(1) },
          ),
          h(
            NText,
            { depth: 3, class: "strength-label" },
            { default: () => getStrengthLabel(row.strength) },
          ),
        ]),
      ]);
    },
...
const loadIndustryData = async (): Promise<void> => {
  ...
  try {
    ...
    error.value =
      err instanceof Error ? err.message : "加载行业数据失败，请稍后重试";
  ...
  <NLayout class="industry-strength-page">
    ...
    <NLayoutContent class="main-content">
      ...
      <NCard class="industry-card" title="行业强弱排名">
        ...
        <!-- 空状态显示 -->
        <div
          v-else-if="!loading && industries.length === 0"
          class="empty-state"
        >
