...
const saveBasket = async () => {
  ...
  try {
    ...
    router.push({ path: "/trade/workspace", query: { tab: "baskets" } });
  } catch (error) {
...
const basketId = computed(() => route.params.id as string | undefined);
isEditMode.value = !!basketId.value;

onMounted(() => {
  if (basketId.value) {
    getBasketDetail(basketId.value);
  }
});
</script>
