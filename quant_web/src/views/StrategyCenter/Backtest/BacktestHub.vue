  ...
  <div class="hub-page scrollbar-hide">
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <p class="page-description">执行策略回测，对比多策略表现，配置回溯周期，验证策略有效性</p>
        </div>
    ...
    <div class="main-content">
      <template v-if="loading">
        <div class="content-section">
          ...
          <n-grid :x-gap="16" :y-gap="16" :cols="2" responsive="screen">
            <n-grid-item v-for="i in 2" :key="i" :class="tokens.motion.stagger" :style="{ animationDelay: `${(i - 1) * 0.08}s` }">
      ...
      <template v-else>
        ...
        <div class="content-section">
          ...
          <n-grid :x-gap="16" :y-gap="16" :cols="2" responsive="screen">
            <n-grid-item :class="tokens.motion.stagger" style="animation-delay: 0s">
              <n-card :class="[tokens.motion.hover, 'function-card']" @click="router.push('/backtest/studio')">
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon accent"><SmartIcon name="TrendingUpOutline" /></div>
                    <h3 class="function-title">回测工作室</h3>
                  </div>
                  <p class="function-desc">多策略回测对比，参数优化和结果导出，全面评估策略表现</p>
                  <div class="function-stats">
                    <div class="stat-item"><span class="stat-label">最近回测</span><span class="stat-value">{{ stats.recentCount }} 次</span></div>
                    <div class="stat-item"><span class="stat-label">通过率</span><span class="stat-value text-up">{{ stats.passRate }}%</span></div>
                    <div class="stat-item"><span class="stat-label">最近回测时间</span><span class="stat-value">{{ stats.lastBacktest }}</span></div>
                  </div>
            ...
            <n-grid-item :class="tokens.motion.stagger" style="animation-delay: 0.08s">
              <n-card :class="[tokens.motion.hover, 'function-card']" @click="router.push('/research/backtest-period')">
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon purple"><SmartIcon name="Calendar" /></div>
                    <h3 class="function-title">回溯周期</h3>
                  </div>
                  <p class="function-desc">配置和管理回测时间周期，支持多周期组合与导出</p>
                  <div class="function-stats">
                    <div class="stat-item"><span class="stat-label">配置周期</span><span class="stat-value">{{ stats.periodCount }} 个</span></div>
                    <div class="stat-item"><span class="stat-label">默认周期</span><span class="stat-value">{{ stats.defaultPeriod }}</span></div>
                    <div class="stat-item"><span class="stat-label">最早起始</span><span class="stat-value">{{ stats.earliestStart }}</span></div>
                  </div>
        ...
        <!-- 最近回测 -->
        <div class="content-section">
          <h2 class="section-title"><SmartIcon name="Clock" class="title-icon" />最近回测</h2>
          <n-card :class="tokens.surface.card">
            <n-empty v-if="recentBacktests.length === 0" description="暂无回测记录" />
            <div v-else class="activity-list">
              <div v-for="(item, idx) in recentBacktests" :key="idx" class="activity-item" @click="router.push(`/backtest/report/${item.id}`)" style="cursor: pointer">
...
const recentBacktests = ref([
  { id: '1', result: 'pass', icon: 'CheckCircle', text: 'CTA趋势跟踪 — 夏普 1.82，最大回撤 12.3%', time: '14:45' },
  { id: '2', result: 'pass', icon: 'CheckCircle', text: '均值回归v2 — 夏普 1.56，最大回撤 8.7%', time: '11:30' },
  { id: '3', result: 'fail', icon: 'CloseCircle', text: '动量突破 — 夏普 0.43，最大回撤 28.1%', time: '昨天 16:10' },
  { id: '4', result: 'pass', icon: 'CheckCircle', text: '多因子Alpha — 夏普 2.10，最大回撤 9.5%', time: '昨天 10:20' },
])
...
.activity-icon {
  ...
  &.pass { background: rgba(0, 230, 118, 0.12); color: var(--color-success, #18a058); }
  &.fail { background: rgba(255, 82, 82, 0.12); color: var(--color-error, #FF5252); }
}
