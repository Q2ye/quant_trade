  ...
  <div class="hub-page bg-gradient-mesh bg-noise">
    ...
    <div class="main-content">
      ...
      <template v-else>
          ...
          <n-grid :x-gap="16" :y-gap="16" :cols="4" responsive="screen">
            ...
            >
              <n-card
                ...
                @click="router.push('/performance/strategy')"
              >
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon accent">
                      <SmartIcon name="Trophy" />
                    </div>
                    <h3 class="function-title">策略绩效</h3>
                  </div>
                  <p class="function-desc">
                    单策略绩效分析，收益率、夏普比率、最大回撤等核心指标
                  </p>
                  <div class="function-stats">
                    <div class="stat-item">
                      <span class="stat-label">分析策略</span
                      ><span class="stat-value"
                        >{{ stats.strategyCount }} 个</span
                      >
                    </div>
                    <div class="stat-item">
                      <span class="stat-label">平均年化</span
                      ><span class="stat-value text-up"
                        >{{ stats.avgAnnualReturn }}%</span
                      >
                    </div>
                  </div>
                ...
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon purple">
                      <SmartIcon name="AnalyticsOutline" />
                    </div>
                    <h3 class="function-title">账户绩效</h3>
                  </div>
                  <p class="function-desc">
                    账户整体收益曲线、资金使用效率、净值变化趋势
                  </p>
                  <div class="function-stats">
                    <div class="stat-item">
                      <span class="stat-label">账户净值</span
                      ><span class="stat-value text-up"
                        >¥{{ stats.accountNav }}</span
                      >
                    </div>
                    <div class="stat-item">
                      <span class="stat-label">累计收益</span
                      ><span class="stat-value text-up"
                        >{{ stats.cumulativeReturn }}%</span
                      >
                    </div>
                  </div>
            ...
            >
              <n-card
                :class="[tokens.motion.hover, 'function-card']"
                @click="router.push('/performance/comparison')"
              >
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon info">
                      <SmartIcon name="Scale" />
                    </div>
                    <h3 class="function-title">绩效对比</h3>
                  </div>
                  <p class="function-desc">
                    多策略横向对比，基准对标，排名与相对强弱分析
                  </p>
                  <div class="function-stats">
                    <div class="stat-item">
                      <span class="stat-label">对比组数</span
                      ><span class="stat-value"
                        >{{ stats.comparisonCount }} 组</span
                      >
                    </div>
                    <div class="stat-item">
                      <span class="stat-label">跑赢基准</span
                      ><span class="stat-value text-up"
                        >{{ stats.beatBenchmark }}%</span
                      >
                    </div>
                  </div>
            ...
            >
              <n-card
                :class="[tokens.motion.hover, 'function-card']"
                @click="router.push('/performance/attribution')"
              >
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon danger">
                      <SmartIcon name="Puzzle" />
                    </div>
                    <h3 class="function-title">归因分析</h3>
                  </div>
                  <p class="function-desc">
                    Brinson/Barra 归因，因子暴露分解，收益来源追踪
                  </p>
                  <div class="function-stats">
                    <div class="stat-item">
                      <span class="stat-label">归因模型</span
                      ><span class="stat-value">{{
                        stats.attributionModel
                      }}</span>
                    </div>
                    <div class="stat-item">
                      <span class="stat-label">最近分析</span
                      ><span class="stat-value">{{
                        stats.lastAttribution
                      }}</span>
                    </div>
                  </div>
        ...
        <div class="content-section">
          <h2 class="section-title">
            <SmartIcon name="Clock" class="title-icon" />最近分析
          </h2>
          <n-card :class="tokens.surface.card">
            <n-empty
            ...
            <div v-else class="activity-list">
              <div
                v-for="(item, idx) in recentAnalyses"
                :key="idx"
                class="activity-item"
...
const recentAnalyses = ref([
  {
    type: "strategy",
    icon: "Trophy",
    text: "CTA趋势跟踪 绩效分析完成 — 夏普 1.82",
    time: "14:30",
    link: "/performance/strategy",
  },
  {
    type: "attribution",
    icon: "Puzzle",
    text: "多因子Alpha 归因分析 — 行业配置贡献 +3.2%",
    time: "11:20",
...
const loadData = async () => {
  ...
  try {
    ...
    if (perf) {
      const p = perf as any;
      if (p.total_return != null)
        stats.value.cumulativeReturn = p.total_return * 100;
      if (p.annual_return != null)
        stats.value.avgAnnualReturn = p.annual_return * 100;
      if (p.total_asset != null)
        stats.value.accountNav = p.total_asset.toLocaleString();
    }
  } catch {
    error.value = true;
  } finally {
