    ...
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <p class="page-description">探索量化因子、管理因子库，为策略构建提供数据驱动的研究基础</p>
        </div>
        ...
        <div class="content-section">
          ...
          <n-grid :x-gap="16" :y-gap="16" :cols="2" responsive="screen">
            <n-grid-item :class="tokens.motion.stagger" style="animation-delay: 0s">
              <n-card :class="[tokens.motion.hover, 'function-card']" @click="router.push('/research/factor-research')">
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon purple"><SmartIcon name="Flask" /></div>
                    <h3 class="function-title">因子探索</h3>
                  </div>
                  <p class="function-desc">快速测试和比较量化因子，发现有效的 alpha 因子</p>
                  <div class="function-stats">
                    <div class="stat-item"><span class="stat-label">最近研究</span><span class="stat-value">{{ stats.researchCount }} 个因子</span></div>
                    <div class="stat-item"><span class="stat-label">有效因子</span><span class="stat-value text-up">{{ stats.activeCount }} 个</span></div>
                    <div class="stat-item"><span class="stat-label">最近更新</span><span class="stat-value">{{ stats.researchUpdateTime }}</span></div>
                  </div>
            ...
            <n-grid-item :class="tokens.motion.stagger" style="animation-delay: 0.08s">
              <n-card :class="[tokens.motion.hover, 'function-card']" @click="router.push('/research/factor-library')">
                <div class="function-inner">
                  <div class="function-head">
                    <div class="function-icon accent"><SmartIcon name="Options" /></div>
                    <h3 class="function-title">因子库</h3>
                  </div>
                  <p class="function-desc">管理量化因子库，支持新建、导入导出和批量操作</p>
                  <div class="function-stats">
                    <div class="stat-item"><span class="stat-label">入库因子</span><span class="stat-value">{{ stats.libraryCount }} 个</span></div>
                    <div class="stat-item"><span class="stat-label">分类覆盖</span><span class="stat-value">{{ stats.categoryCount }} 类</span></div>
                    <div class="stat-item"><span class="stat-label">最近入库</span><span class="stat-value">{{ stats.libraryUpdateTime }}</span></div>
                  </div>
        ...
        <div class="content-section">
          ...
          <n-card :class="tokens.surface.card">
            <div class="quick-actions">
              <n-button type="primary" @click="router.push('/research/factor-research')">
...
const recentActivities = ref([
  { type: 'research', icon: 'Flask', text: '因子 "动量反转" 测试完成，夏普比率 1.82', time: '10:30' },
  { type: 'library', icon: 'Options', text: '导入 3 个技术指标因子到因子库', time: '09:45' },
  { type: 'research', icon: 'Flask', text: '因子 "波动率聚集" 研究已保存', time: '昨天 16:20' },
  { type: 'library', icon: 'Options', text: '因子 "流动性冲击" 分类更新', time: '昨天 14:10' },
])
...
.activity-icon {
  ...
  &.research { background: rgba(124, 111, 247, 0.12); color: var(--color-purple, #7C6FF7); }
  &.library  { background: rgba(68, 138, 255, 0.12); color: var(--color-primary, #448AFF); }
}
