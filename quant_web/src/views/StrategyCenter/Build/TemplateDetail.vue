          ...
          <div class="title-section">
            <div>
              ...
              <p class="page-description">
                <n-tag :type="builtinTypeTag(template.template_type) as any" size="tiny">{{ builtinTypeLabel(template.template_type) }}</n-tag>
                <n-tag v-if="template.is_builtin" type="info" size="tiny" :bordered="false" style="margin-left:4px">内置</n-tag>
              </p>
            </div>
          </div>
          <div class="header-actions">
            <n-button class="action-btn" @click="load" quaternary><template #icon><Icon icon="material-symbols:refresh" /></template></n-button>
            <n-button class="action-btn" @click="router.back()" quaternary><template #icon><Icon icon="material-symbols:arrow-back" /></template></n-button>
          </div>
        </div>
      </div>
...
const paramEntries = computed(() => {
  ...
  return Object.entries(params).map(([key, value]) => ({
    ...
    step: typeof value === 'number' && Number.isInteger(value) ? 1 : 0.1,
  }));
});
...
const FLAT_PARAM_LABELS: Record<string, string> = {
  ...
  // 行业轮动 V2 参数
  buffer_rank: '卖出缓冲排名', cooling_period: '冷却期(天)',
