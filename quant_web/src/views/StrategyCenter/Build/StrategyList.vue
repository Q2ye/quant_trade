...
const BUILTIN_META: Record<string, { name: string; desc: string; params?: Record<string,string> }> = {
  ...
  MultiAssetRotationStrategy: { name: '多资产ETF轮动', desc: 'A股宽基+跨境+商品+债券 ETF轮动，对数线性回归动量×R²选最强ETF，全仓持有一只。',
    params: { etf_pool: 'ETF候选池', momentum_days: '动量回归窗口(天)',
      rsrs_window: 'RSRS计算窗口', rsrs_lookback: 'RSRS Beta回溯天数',
      rsrs_beta_window: 'RSRS Beta滚动窗口', volume_check_days: '量异常检测周期(天)',
      volume_threshold: '量异常阈值(倍)', rebalance_frequency: '调仓频率(天)',
      top_n: '持仓数量', stop_loss: '止损比例',
