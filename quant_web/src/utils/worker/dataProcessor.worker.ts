...
self.addEventListener(
  "message",
  async (e: MessageEvent<{ task: string; payload: any }>) => {
...
}): Promise<{ meta: any; data: StockData[] }> {
  ...
  data.forEach((item, index) => {
    ...
    if (item.pre_close !== undefined && item.pct_chg !== undefined) {
      const expectedChange =
        ((item.close - item.pre_close) / item.pre_close) * 100;
      if (Math.abs(expectedChange - item.pct_chg) > 0.01) {
        warnings.push(
          `行 ${index + 1}: 涨跌幅计算不一致 (计算值: ${expectedChange.toFixed(4)}, 记录值: ${item.pct_chg})`,
        );
      }
    }
  });
